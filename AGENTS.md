...
- Host listeners are unreachable: StartOS firewalls raw host ports, so `downloadWorld.ts` stages ZIPs into `main/downloads/` served by the `busybox` sidecar (`downloads` daemon + `downloads` interface). Serve LAN-reachable files only through declared interfaces. Preferred external ports are not guaranteed (the OS falls back to a random high port) — never promise a port number in user-facing text; the Interfaces tab is authoritative.
