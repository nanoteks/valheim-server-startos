...
- Host listeners are unreachable: StartOS firewalls raw host ports, so `downloadWorld.ts` stages ZIPs into `main/downloads/` served by the `busybox` sidecar (`downloads` daemon + `downloads` interface, external 28763). Serve LAN-reachable files only through declared interfaces.
